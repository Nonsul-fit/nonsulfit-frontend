...
const UnivDetailSummary = ({ currentUniversity }: UnivDetailSummaryProps) => {
  ...
  const { university, campus, summary, explanations } = currentUniversity;

  const nonsulRatio = summary.essayRatio;
  const naesinRatio = summary.naesinRatio;
    ...
    >
      ...
      >
        {[
          ...
          { label: "시험일", value: summary.examDateText },
          {
            label: "문제유형",
            value:
          ...
          { label: "최저", value: summary.csatRequirement },
        ].map((item, idx) => (
      ...
      >
        ...
        <div className="flex flex-col justify-center space-y-1 text-right lg:text-left min-w-[75px]">
          ...
          <div
            className={`flex items-center gap-2 justify-end lg:justify-start transition-opacity duration-200 ${
              naesinRatio > 0
                ? "opacity-100"
